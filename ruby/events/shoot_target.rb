...
class Shoot < SoccerEvent
  ...
  def final_condition(scene1,scene2)
    ...
    if attackteam == 1
      ...
      if right_possibilities.include?(region_ending)
        ...
        return "Shoot by team: " + attackteam.to_s + " by player " + iplayer.getNumber.to_s
        #return true
      end
    else
  ...
  def constrain(scene1,scene2)
    ...
    iplayer = istate[:from]
    player_kicking = is_kicking?(scene1,scene2)
    if player_kicking.nil? 
      return true
    else
      # load player
      player_kicking = last_ball.closer(scene1.getPlayers)
      return true if player_kicking == iplayer
      return false
    end
